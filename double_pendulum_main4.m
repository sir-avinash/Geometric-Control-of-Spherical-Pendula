function double_pendulum_main4

addpath('C:\Users\Avinash\Dropbox\CMU_lap\Misc\My Toolbox');

q10 = Rx(-10*pi/180)*Ry(-20*pi/180)*[0 0 1]';
w10 = [0 -2 0]';
...
% hold on
% [q2d,w2d]=double_pendulum_constraints(q1,w1);
...
% eq2(:,i) = cross2(q2d,q2);
% ew2(:,i) = w2 + (hat(q2)^2)*w2d; %-(hat(q2)^2)*(w2-w2d);
% % ewt(:,i) = -(hat(q2)^2)*(w2-w2d);
% % norm(w2)
% 
% normeq(i) = norm(eq2(:,i));
% normew(i) = norm(ew2(:,i));
% % normewt(i) = norm(ewt(:,i));
...
B=  [zeros(3);1*eye(3)+0*hat(q2)];  %   [zeros(3);eye(3)];   ;
B2 = [zeros(3);0*eye(3)+1*hat(q2)];
...
tmp=-hat(q2)^2;  %% transport map eye(3);
eps = 0.5;%1/sqrt(10);
kq2 = 10/eps^2;
kw2 = 10.1/eps;
[q2d,w2d,R]=double_pendulum_constraints(q1,w1);
eq2 = cross2(q2d,q2);
ew2 = w2 - tmp*w2d; % tmp2*(w2-w2d);


v=-kq2*eq2-kw2*ew2;
% Lf2h = Fq2 + tmp*(hat(q2d)^2*Fq1 + hat(w2d)*w1) + dot(q2,w2d)*dq2 ;
% LgLfh = Gq2 + tmp*(hat(q2d)^2*Gq1);
% Lf2h = (Fq2-hat(dq2)*hat(q2)*w2-hat(q2)*hat(dq2)*w2)+(- tmp*R*Fq1 + dot(q2,w2d)*dq2 + tmp*dot(dq2,w2d)*q2);
Lf2h = (Fq2-tmp*R*Fq1) + dot(q2,w2d)*dq2 + 0*dot(dq2,w2d)*q2 ;
LgLfh = Gq2 - tmp*R*Gq1;
u = pinv(LgLfh)*(0*v-Lf2h);

%  [dot(ew2,q2) dot(-Lf2h,q2) dot(LgLfh*u,q2)]
...
[q20,w20]=double_pendulum_constraints(q10,w10);
q20=pertq*q20;
w20=pertw*w20;
