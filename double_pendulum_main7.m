function double_pendulum_main7
...
d.m = 1;
d.l = 0.2;
d.theta = -20*(pi/180); % Pitch := Rotation about Y-axis; Start with some negative deflection
d.phi = 0*(pi/180); % Roll := Rotation about X-axis; Start with some positive displacement
...
dx =[dq1;dq2;dw];
[acos(dot(q1,q2))*(180/pi) norm(dw(1:3)) norm(dw(4:6)) norm(u)]
end
...
Lf2h = Fq2 + tmp*R*hat(q1)^2*Fq1 + 0*tmp*R*hat(q1)*hat(dq1)*w1 + 0*hat(q2)*hat(dq2)*w2d + hat(dq2)*hat(q2)*w2d  ;
